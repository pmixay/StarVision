...
- **Автоматическая подгрузка TLE с CelesTrak** с явной маркировкой источника (demo / live / fallback)
...
- **17 российских КА** в каталоге (все активные): Декарт, НОРБИ, Ярило-1, CubeSX-HSE, ReshUCube-2, VIZARD-meteo, УмКА-1, НОРБИ-2, CubeSX-HSE-3, Монитор-2, Ярило-3, СамСат-Ионосфера, TUSUR GO, RTU MIREA-1, Горизонт, ASRTU-1, Vizard-ion
...
- **Оптимизированный рендеринг** — пулинг объектов, дросселирование, сокращение per-frame работы
...
| Высота орбиты | TLE / 400–2000 км | TLE = live/demo источник; иначе виртуальные Walker-орбиты |
| Источник TLE | Встроенные / CelesTrak | Выбор между встроенным demo-набором и live CelesTrak |
| Наклонение | 0–180° | Наклонение виртуальной группировки |
...
| **SPUTNIX** | CubeSX-HSE (47952), CubeSX-HSE-3 (57178), VIZARD-meteo (57189) | ДЗЗ, мониторинг Арктики, технологические эксперименты | 3U |
| **Геоскан** | Vizard-ion (61749) | Плазменный двигатель VERA, радиозатменное зондирование ионосферы | 3U |
| **НИИЯФ МГУ** | Монитор-2 (57184) | Рентген/гамма-наблюдения | 3U |
| **Space-Pi** | ReshUCube-2 (57168), TUSUR GO (61782), RTU MIREA-1 (61785), Горизонт (61757), ASRTU-1 (61781) | Образовательные, научные, IoT/МСС | 3U |
...
| Метод | URL | Описание |
|---|---|---|
| GET | `/api/satellites` | Список всех 17 КА с метаданными |
| GET | `/api/health` | Статус backend и кэша |
...
| GET | `/api/tle/status` | Статус кэша CelesTrak |
| POST | `/api/tle/refresh` | Принудительное обновление TLE с CelesTrak |
| GET | `/api/orbit/{norad_id}` | Орбитальный трек (120 точек, шаг 60с) |
| GET | `/api/orbits` | Пакетные орбитальные треки |
| GET | `/api/links?comm_range_km=2000` | МСС с LOS-проверкой |
| GET | `/api/orbital-elements/{norad_id}` | Кеплеровы элементы орбиты |
| GET | `/api/collisions` | Прогноз сближений для real TLE или virtual Walker |
| GET | `/api/optimize-planes` | Оптимизатор Walker-δ |
...
- **CelesTrak** — https://celestrak.org — live-загрузка TLE для российских КА
- Данные кэшируются на 1 час, с fallback на встроенный demo orbital set при недоступности сервиса
...
- Live орбитальные данные берутся из открытых публичных источников CelesTrak; встроенный fallback явно помечен как demo orbital set
