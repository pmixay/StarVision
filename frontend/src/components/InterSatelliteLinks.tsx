...
function computeVirtualPositions(
  ...
  for (let i = 0; i < count; i++) {
    result[i] = computeVirtualECI(i, count, altitudeKm, t, planes, inclinationRad);
  }
  return result;
}
...
export function InterSatelliteLinks({ tleData, satelliteConstellations }: InterSatelliteLinksProps) {
  ...
  useFrame(() => {
    ...
    if (orbitAltitudeKm > 0) {
      ...
      const virt = computeVirtualPositions(satelliteCount, orbitAltitudeKm, simTime, orbitalPlanes, inclRad);
      eciPositions = virt
        .map((p, i) => ({
          norad_id: 90000 + i,
